...
\begin{multline}
    \bproof =   \underbrace{ \bhash+ t\cdot MP(\frac{n}{\folds[0]},\sbtch,  q,\bhash)}_{\text{Initial round}} + \\
    +\underbrace{\sum_{1\leq i \leq r_{FRI}-2} \left(\bhash+
     t\cdot MP(\frac{n}{\prod_{0\leq j \leq  i} \folds[j]},\sbtch, q,\bhash)\right)}_{\text{Folding rounds but last }}+\\
    +\underbrace{ \left(\bhash+
     t\cdot MP(\frac{n}{ \prod_{0\leq j \leq  r_{FRI}-1} \folds[j]},\sbtch,q,\bhash)\right)}_{\text{Last folding round }}
    \end{multline}
